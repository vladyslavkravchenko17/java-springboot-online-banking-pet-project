...
<@c.page>
    <div align="center" class="d-flex justify-content-center align-items-center" style="margin-top: 70px;">
        <#if cards?hasContent>
            <form action="/create-deposit" method="post" onsubmit="return confirm('Do you want to make deposit?');">
                <h3 class="font-weight-light text-uppercase">Make Deposit:</h3>
                <#if message??>
                ...
                <div class="form-group">
                    <input type="number" class="form-control rounded-pill form-control-lg" placeholder="Deposit sum"
                           name="sum">
                </div>
                <div class="mb-1 forgot-link form-group d-flex justify-content-between align-items-center">
                    ...
                    <a href="/info/deposit">Read deposit rules.</a>
                </div>
                <input type="hidden" name="number" value="${card.number}">
                <input type="hidden" name="_csrf" value="${_csrf.token}"/>
                <button class="btn mt-5 rounded-pill btn-lg btn-custom btn-block text-uppercase">Make</button>
                <div class="form-group mt-2">
                    Deposit percent is 10% for 12 months.
                </div>
            </form>
        <#else>
